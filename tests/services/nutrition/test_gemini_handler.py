...
# We need to mock the class and the get_model method
@patch('src.services.nutrition.gemini_handler.AIModelManager')
class TestGeminiHandler(unittest.TestCase):

    def test_choose_best_usda_match_single_candidate(self, MockAIModelManager):
        """Test that the single candidate ID is returned directly."""
        query = "apple"
        candidates = [{'fdcId': 123, 'description': 'Apple, gala'}]
        result = gemini_handler._choose_best_usda_match(query, candidates)
        self.assertEqual(result, 123)
        MockAIModelManager.get_model.assert_not_called() # Model should not be needed

    def test_choose_best_usda_match_no_candidates(self, MockAIModelManager):
        """Test that None is returned for empty candidates."""
        result = gemini_handler._choose_best_usda_match("food", [])
        self.assertIsNone(result)
        MockAIModelManager.get_model.assert_not_called()

    def test_choose_best_usda_match_success(self, MockAIModelManager):
        """Test successful selection by Gemini."""
        mock_model_instance = MagicMock()
        mock_response = MagicMock()
        mock_response.text = "   FDC ID: 5678   "
        mock_model_instance.generate_content.return_value = mock_response
        MockAIModelManager.get_model.return_value = mock_model_instance

        query = "bread"
        candidates = [
            {'fdcId': 1234, 'description': 'Bread, white'},
            {'fdcId': 5678, 'description': 'Bread, whole wheat'}
        ]
        ...
        mock_model_instance.generate_content.assert_called_once_with(
            ANY, # Don't need to check the exact prompt string here
            request_options={'timeout': 30}
        )

    def test_choose_best_usda_match_gemini_returns_bad_id(self, MockAIModelManager):
        """Test fallback when Gemini returns an ID not in the list."""
        mock_model_instance = MagicMock()
        mock_response = MagicMock()
        mock_response.text = "9999"
        mock_model_instance.generate_content.return_value = mock_response
        MockAIModelManager.get_model.return_value = mock_model_instance

        query = "milk"
        candidates = [
            {'fdcId': 111, 'description': 'Milk, whole'},
            {'fdcId': 222, 'description': 'Milk, skim'}
        ]
        first_candidate_id = candidates[0]['fdcId']

        result = gemini_handler._choose_best_usda_match(query, candidates)
        self.assertEqual(result, first_candidate_id) # Should default to first
        MockAIModelManager.get_model.assert_called_once_with('nutrition')
        mock_model_instance.generate_content.assert_called_once()

    def test_choose_best_usda_match_gemini_unparsable(self, MockAIModelManager):
        """Test fallback when Gemini returns unparsable text."""
        mock_model_instance = MagicMock()
        mock_response = MagicMock()
        mock_response.text = "I think the best match is..."
        mock_model_instance.generate_content.return_value = mock_response
        MockAIModelManager.get_model.return_value = mock_model_instance

        query = "cheese"
        candidates = [
            {'fdcId': 333, 'description': 'Cheese, cheddar'},
            {'fdcId': 444, 'description': 'Cheese, mozzarella'}
        ]
        first_candidate_id = candidates[0]['fdcId']
    ...
    def test_choose_best_usda_match_gemini_api_error(self, MockAIModelManager):
        ...
        query = "yogurt"
        candidates = [
            {'fdcId': 555, 'description': 'Yogurt, plain'},
            {'fdcId': 666, 'description': 'Yogurt, greek'}
        ]
        first_candidate_id = candidates[0]['fdcId']
